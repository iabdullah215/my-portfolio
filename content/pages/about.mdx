---
title: About
description: Muhammad Abdullah aka Hwat Sauce
---

Creative, passionate, and self-taught individual with a love for *Offensive Security*, *Penetration Testing*, *Red Teaming*, *Reverse Engineering*, *CTFs* and *Little bit of Crypto*. Willing to work with others in the hopes of learning new things and creatively providing solutions to complex problems and collaborating with them.

By day, I'm a coffee-fueled coder and nerd, hammering out lines of code like there's no tomorrow. But when the sun sets, the real fun begins. I don my virtual cape, pull on my digital gloves, and dive deep into the cyber-verse. I'm not just a hacker — I safeguard the digital landscape.

I dance through encryption like it's a rhythm game, always two steps ahead of the cyber crooks. Don't worry, though — I mostly focus on keeping things secure.

In this age of digital goldmines, I'm the silent protector, watching over your data while you sleep. If there's a breach to be patched or a botnet to be busted, you can count on me. So if you need a partner in code or just someone to talk shop about the latest infosec trends, I'm your go-to. Here's to being a keyboard ninja by night and a security enthusiast by day!

## Skills

- **Programming Languages**: Python, C++, C#, Java, Bash, and Assembly.
- **Security Tools**: Docker, Metasploit, Burp Suite, Nessus, SQLMap, Ligolo-NG, BloodHound, CrackMapExec, Certipy, Bloodly-AD, Impacket, IDA Pro, Ghidra, Farida, GDB, Volitility, and Nmap.
- **Platforms**: Linux, Windows, MacOS, AWS, GCP, Azure, Docker, GitHub, GitLab, and Cloudflare.
- **Blogging**: As a hobby, I periodically blog about cybersecurity. You can find my posts on [Medium](https://medium.com/@iabdullah_215).

## Experience

- **[ABHI Microfinance Bank, Ltd.](https://abhibank.com/)** — Islāmābād, Pakistan [Full Time - On Site]
  - **Asst. Mgr. System Security & QA** (Jul 2026 - Present)
  - **Officer System Security & QA** (Dec 2025 - Jul 2026)
- **Penetration Tester** at [Pakistan National CERT](https://pkcert.gov.pk/) (May 2025 - August 2025) [Intern - On Site]
- **Security Challenge Developer** at [Trustline](https://www.trustline.sa/) (April 2024 - Sep 2024) [Frelance - Remote]
- **Cyber Security Analyst** at [NCCS - National Center Of Cybersecurity](https://www.nccs.pk/) in collaboration with [Cybericks](https://cybericks.com/) (June 2024 - Sep 2024) [Intern - Hybrid]

## Education

- **Masters in Information Security** from [National University of Sciences & Technology (NUST)](https://nust.edu.pk) (Sep 2026 - Present)
- **Bachelors in Cyber Security** from [Air University, Islamabad](https://au.edu.pk/) (Sep 2022 - Jul 2026)

## Certifications

- [Certified in Cyber Security (CC)]()
- [Certified ISO/IEC 27001 Information Security Associate](https://www.skillfront.com/Badges/96244120273692)
- [INE Certified Cloud Associate (ICCA)](https://certs.ine.com/3d857008-1603-41dc-a5ea-1e0c590cbe33#gs.b8suwa)
- [INE Certified Jr. Penetration Tester (eJPT)](https://certs.ine.com/3d857008-1603-41dc-a5ea-1e0c590cbe33#gs.b8suwa)
- [Certified Network Security Practitioner (CNSP)]()
- [Certified Red Team Analyst (CRTA)](https://www.credential.net/724e06f3-0a39-402b-9ced-b9c3953aa3e3#acc.eXn7Jl3j)
- [Certified Penetration Testing Specialist (CPTS)](https://www.hackthebox.com/certificates)

## Publications and Advisories:

- [Evolution Cybercrime — Key Trends, Cybersecurity Threats, and Mitigation Strategies from Historical Data](https://www.mdpi.com/2813-2203/4/3/25) [MDPI] [**Listed in the Editor’s Choice Articles**](https://www.mdpi.com/journal/analytics/editors_choice)
- [GHSA-f2v7-35mm-3hx7 — manager/maven-wrapper: Command injection could be possible via unescaped `distributionType`](https://github.com/renovatebot/renovate/security/advisories/GHSA-f2v7-35mm-3hx7) - [Renovate]
- [GHSA-v85g-rq5w-c46q — manager/mix: Command injection could be possible via unescaped `organization`](https://github.com/renovatebot/renovate/security/advisories/GHSA-v85g-rq5w-c46q) - [Renovate]
- [CVE-2026-71184 — GHSA-7w2r-xwp6-mh6c — Gitea Actions: fork pull request approval bypass via `pull_request_review_comment` (missing `WithPullRequest`)](https://github.com/go-gitea/gitea/security/advisories/GHSA-7w2r-xwp6-mh6c) - [Gitea]
- [CVE-2026-66874 — GHSA-5xp7-r6cr-39ff — A fork pull request author can satisfy a required Actions status check without any workflow running](https://github.com/go-gitea/gitea/security/advisories/GHSA-5xp7-r6cr-39ff) - [Gitea]
- [CVE-2026-78433 — GHSA-frpv-2xgv-wxpq — Attachments created before the January 2026 cutoff skip the cross repository check, so a private repository's attachment is served through any public repository's path](https://github.com/go-gitea/gitea/security/advisories/GHSA-frpv-2xgv-wxpq) - [Gitea]
- [GHSA-f689-h8m7-3jp2 — `ContainerizationOCI` accepts unvalidated OCI descriptor digests, enabling path traversal in the local content store](https://github.com/apple/containerization/security/advisories/GHSA-f689-h8m7-3jp2) - [Apple Containerization]
- [CVE-2026-70407 — GHSA-ww9q-7j5q-hrv5 — Token scope bypass on `GET /api/v1/orgs`: the `read:organization` check ran after the handler that answered the request](https://github.com/go-gitea/gitea/security/advisories/GHSA-ww9q-7j5q-hrv5) - [Gitea]
- [GHSA-5293-mq8x-g3xj — Malicious OpenAPI documents can cause arbitrary code generation, executed when anyone imports the generated client](https://github.com/openapi-generators/openapi-python-client/security/advisories/GHSA-5293-mq8x-g3xj) - [openapi-python-client]

## Achievements

- **2nd Place** at [Hack Arena CTF]() organized by [UET Peshawar](https://www.uetpeshawar.edu.pk/), onsite (2025)
- **Top 50** at [Black Hat MEA 2025 CTF Quals](https://blackhatmea.com/) organized by [Flagyard](https://flagyard.com/), online (2025)
- **4th Place** at [Cyber Muhafiz CTF'25](https://neocyberrange.org/) organized by [Pakistan National CERT](https://pkcert.gov.pk/), onsite (2025)
- **4th Place** at [SofTech CTF'25](https://softecnu.org/) organized by [Fast University Lahore](https://lhr.nu.edu.pk/), onsite (2025)
- **50th Global Rank** at [PicoCTF'25](https://picoctf.org/competitions/2025-spring.html) organized by [PicoCTF](https://picoctf.org/), online (2025)
- **Top 50** at [Black Hat MEA 2024 CTF Finalists](https://blackhatmea.com/) organized by [Flagyard](https://flagyard.com/), onsite (2024)
- **98th Global Rank** at [CSWA CTF Qualifiers 2024](https://www.csaw.io/ctf) organized by [New York University](https://www.nyu.edu/), online (2024)
- **4th Place** at [AirTech CTF'24](https://auair.tech/) organized by [Air University Islamabad](https://www.au.edu.pk/), onsite (2024)
- **5th Place** at [SofTech CTF'24](https://softecnu.org/) organized by [Fast University Lahore](https://lhr.nu.edu.pk/), onsite (2024)
- **10th Place** at [Digital Pakistan Cyber Security Hackathon](https://cyberhackathon.pk/) organized by [IGNITE](https://ignite.org.pk/), onsite in Islamabad (2023)
- **5th Place** at CyberQuest CTF'23 organized by Sir Syed CASE Institute of Technology, onsite (2023)
- **4th Place** at BlitzStorm CTF'23 organized by [Blitzstorm](https://www.linkedin.com/company/blitzstorm/), online (2024)
- **8th Place** at [NasCon'23](https://ctftime.org/event/1894/) organized by [FAST University Islamabad](http://isb.nu.edu.pk/), onsite (2023)
- **4th Place** at CyberStorm organized by Air University, onsite in Islamabad (2023)
- Ranked in the **top 6%** on [TryHackMe](https://tryhackme.com/p/M.n.M), online (2022 - Present)

## Projects

- [**DeepTrace**](https://github.com/iabdullah215/DeepTrace-IPDR): DeepTrace is an Internet Protocol Detail Record (IPDR) system that captures, parses, and visualizes network traffic data at flow level. It supports both offline PCAP file analysis and real-time packet capture, with a multi-layer security analysis engine capable of detecting 23 distinct attack types.
- [**pathhound-mcp**](https://github.com/iabdullah215/pathhound-mcp): An MCP (Model Context Protocol) server that lets an LLM reason over an Active Directory attack graph already collected by BloodHound — shortest paths to Tier 0, blast radius from a principal, the highest-leverage choke points to remediate, and defender-facing remediation text — with every action scoped to an authorized engagement and logged.
- [**ActiveDecoy**](https://github.com/iabdullah215/ActiveDecoy): An automated ITDR (Identity Threat Detection and Response) framework designed to orchestrate and deploy believable Active Directory honey-objects for proactive lateral movement detection.
- [**GPO-Audit**](https://github.com/iabdullah215/GPO-Audit): GPO-Audit audits Microsoft Group Policy Object (GPO) reports exported via Get-GPOReport (HTML or XML). It focuses on identifying common hardening gaps, risky delegation, and unresolved (orphaned) security principals, and it can export results and compare drift against a baseline.
- [**HeepHound**](https://github.com/iabdullah215/HeapHound): HeapHound is a Python-based tool built with my dear friend [Muhammad Munib](https://www.linkedin.com/in/muhammad-munib-nawaz-7b8129292/) and [Radeel Ahmed](https://www.linkedin.com/in/radeel-ahmad-866a50293/) for analyzing Java heap dump files (`.hprof`) to extract sensitive data, assess risks, and generate forensic reports. It identifies credentials, tokens, and other artifacts using built-in heuristics, producing reports in `JSON`, `HTML`, and text formats. Ideal for security investigations, malware analysis, and incident response.
- [**NetEye**](https://github.com/iabdullah215/NetEye): Net-Eye is an all-in-one Python-based network reconnaissance tool that detects live hosts, scans ports, grabs banners, resolves DNS, performs GeoIP lookups, and runs detailed Nmap scans. It also identifies known vulnerabilities based on discovered services, helping users assess and secure their networks effectively.
- [**hwatlib**](https://github.com/iabdullah215/hwatlib): A practical pentesting and exploitation python library with wrappers for recon, web enumeration, reverse shells, and privilege escalation.
- [**Blockchain Implementation**](https://github.com/mun1bxD/BlockChain): Developed a Java-based blockchain with features like user authentication, transaction tracking, and dynamic coin rate fluctuation. This project was a collaborative effort with [Muhammad Munib](https://www.linkedin.com/in/muhammad-munib-nawaz-7b8129292/), [Radeel Ahmed]

## Additional Experiences

- **Student Advisor** at [Air University Cyber Security Society](https://aucssociety.com/) (Feb 2026 - July 2026)
- **Technical Secretary** at [Air University Cyber Security Society](https://aucssociety.com/) (Dec 2024 - Feb 2026)
- **Director Developer** at [AirTech'25 CTF](https://www.instagram.com/airtech.25/?hl=en) (May 2025)
- **CTF Challenge Developer** at [Air University Cyber Security Society](https://aucssociety.com/) (Dec 2023 - Dec 2024)
- **Member - Cyber Research Club** at [Air University Cyber Security Society](https://aucssociety.com/) (Feb 2024 - Dec 2024)
- **Core Member - Awareness Club** at [Air University Cyber Security Society](https://aucssociety.com/) (Jan 2023 - June 2023)

## Contact Me

- **Email**: `muhammadabdullah8040@gmail.com`
- **Twitter**: [@iabdullah_215](https://twitter.com/iabdullah_215)
- **LinkedIn**: [Muhammad Abdullah](https://www.linkedin.com/in/muhammad-abdullah-691a1026a/)
- **GitHub**: [iabdullah215](https://github.com/iabdullah215)
- **Discord**: `n0tabdu11ah`
